...
DJANGO_MAPWIDGETS_CONTAINER_NAME = os.environ.get(
    "DJANGO_MAPWIDGETS_CONTAINER_NAME", "django_mapwidgets"
)
POSTGRES_CONTAINER_NAME = os.environ.get(
    "DJANGO_MAPWIDGETS_CONTAINER_NAME", "mapwidget_postgres"
)
...
def run_on_docker(command):
    local(
        'docker exec -it {} /bin/bash -c "{}"'.format(
            DJANGO_MAPWIDGETS_CONTAINER_NAME, command
        )
    )
...
def docker_covarage_tests():
    run_on_docker(
        'cd tests/testapp;coverage run --source="../../mapwidgets" manage.py test;coverage report'
    )
